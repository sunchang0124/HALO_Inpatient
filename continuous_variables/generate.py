...
labelProbs = pickle.load(open('data/labelProbs.pkl', 'rb'))
    ...
    with torch.no_grad():
        with torch.cuda.amp.autocast():
            for _ in range(length-1):
                raw_model = model.module if hasattr(model, 'module') else model
                prev = raw_model.sample(torch.cat((prev, empty), dim=1), sample)
...
def convert_ehr(ehrs, index_to_code=None):
    ...
    for i in range(len(ehrs)):
        ...
        if index_to_code is not None:
            labels_output = [index_to_code[idx] for idx in np.nonzero(labels_output)[0]]
...
for i in tqdm(range(0, totEHRs, sample_batch_size)):
    bs = min([totEHRs - i, sample_batch_size])
...
pickle.dump(synthetic_ehr_dataset, open('./results/datasets/haloDataset.pkl', 'wb'))
print(f"Saved {len(synthetic_ehr_dataset)} synthetic patients to results/datasets/haloDataset.pkl")
